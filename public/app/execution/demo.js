import { buildDemoCatalogue } from './catalogue.js';
...
/**
 * The five products the demonstration cabinet is built from.
 *
 * All five are JTI's own, because the workflow being shown is a JTI own-shelf audit: the
 * presence and facing rules a field user is checking are rules about JTI product. Competitor
 * SKUs exist in the catalogue and can be recorded on a shelf, they are simply not what this
 * cabinet is measured against.
 */
export const DEMO_HEROES = [
  'TW-JTI-MEVIUS-ORIGINAL',
  'TW-JTI-SEVENSTARS-ORIGINAL',
  'TW-JTI-WINSTON-RED',
  'TW-JTI-CAMEL-FILTERS',
  'TW-JTI-LD-RED',
];
export const COLORS = {
  'TW-JTI-MEVIUS-ORIGINAL': '#39746b',
  'TW-JTI-SEVENSTARS-ORIGINAL': '#446a98',
  'TW-JTI-WINSTON-RED': '#b0894f',
  'TW-JTI-CAMEL-FILTERS': '#8a6594',
  'TW-JTI-LD-RED': '#b76961',
};
/** Brand and variant on separate lines: a full SKU id does not fit one cabinet cell. */
const HERO_LABEL = {
  'TW-JTI-MEVIUS-ORIGINAL': ['MEVIUS', 'ORIGINAL'],
  'TW-JTI-SEVENSTARS-ORIGINAL': ['SEVEN STARS', 'ORIGINAL'],
  'TW-JTI-WINSTON-RED': ['WINSTON', 'RED'],
  'TW-JTI-CAMEL-FILTERS': ['CAMEL', 'FILTERS'],
  'TW-JTI-LD-RED': ['LD', 'RED'],
};
...
export function demoSlots(fixtureId) {
  return Array.from({ length: 20 }, (_, i) => {
    ...
    let sku = DEMO_HEROES[Math.floor((column - 1) / 2)], state = 'product';
    if (fixtureId === 'F2' && row === 1 && column === 1) sku = DEMO_HEROES[1];
    if (fixtureId === 'F2' && row === 1 && column === 3) sku = DEMO_HEROES[0];
...
export function buildTaiwanDemo() {
  ...
  w.catalogue = buildDemoCatalogue();
  ...
  const plan = { id: 'tw-plan-v1', lineageId: 'tw-plan', market: 'TW', code: 'TW-DEMO-CABINET', version: 1, revision: 1, status: 'published', fixtureType: 'regular_cabinet', rows: 2, columns: 10, validFrom: '2026-08-23T04:00:00.000Z', validTo: null, changeNote: 'Workflow demonstration cabinet carrying real JTI products. The 7x30 three-bay geometry of an actual Taiwan convenience-store cabinet is held in the shared Planogram Check workspace; this reduced 2x10 grid exists so each slot stays legible on screen.', updatedAt: '2026-08-23T04:00:00.000Z', updatedBy: 'tw-manager', sourceName: 'Prepared demonstration reference · real products, illustrative layout', slots: demoSlots('F1').map(s => ({ key: s.key, row: s.row, column: s.column, active: true, allowedSkuIds: [s.sku], preferredSkuId: s.sku, critical: true })), requiredSkuIds: [...DEMO_HEROES], facingRules: DEMO_HEROES.map(id => ({ skuId: id, min: 4, max: 4 })) };
...
export function cabinetSvg(fixtureId) {
  const packs = demoSlots(fixtureId).map(s => {
    ...
    const ink = s.sku ? '#ffffff' : '#53616b';
    const [brand, variant] = HERO_LABEL[s.sku] ?? [s.state === 'empty' ? 'EMPTY' : 'OBSCURED', ''];
    return `<g><rect x="${x}" y="${y}" width="${w}" height="${h}" rx="6" fill="${s.state === 'empty' ? '#e7eceb' : color}" stroke="${s.state === 'empty' ? '#8f9997' : '#ffffff'}" stroke-width="2" ${s.state === 'empty' ? 'stroke-dasharray="7 5"' : ''}/><text x="${x + w / 2}" y="${y + h * .38}" text-anchor="middle" fill="${ink}" font-family="Arial,sans-serif" font-size="${s.sku ? 13 : 12}" font-weight="700">${brand}</text><text x="${x + w / 2}" y="${y + h * .6}" text-anchor="middle" fill="${ink}" font-family="Arial,sans-serif" font-size="11">${variant}</text><text x="${x + w / 2}" y="${y + h * .84}" text-anchor="middle" fill="${ink}" font-family="Arial,sans-serif" font-size="11" opacity=".85">${s.key}</text></g>`;
  }).join('');
