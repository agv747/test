...
import { COLORS, preparedProposal, cellBox } from './demo.js';
...
const titles = { 'tw/overview': 'Taiwan · Execution overview', 'tw/audits/new': 'Start cabinet audit', 'tw/audits/detail': 'Cabinet audit', 'tw/planograms': 'Planogram library', 'tw/planograms/detail': 'Reference planogram', 'tw/issues': 'Execution issues', 'admin/ai': 'AI models & connections' };
...
function banner() {
  ...
  return `<div class="rei-scope"><div>${badge(mode === 'demo' ? 'Prepared demonstration' : 'Private workspace', mode === 'demo' ? 'info' : 'good')} <span>${mode === 'demo' ? 'Synthetic outlets & packs · 22 Sep 2026, 12:00 Taipei' : `Signed in as ${esc(actor?.name)} · Taiwan`}</span></div><div class="toolbar">${mode === 'demo' ? button('tw-reset', 'Reset example', '', 'btn--sm') : button('tw-refresh', 'Refresh', '', 'btn--sm')}<select data-action="tw-workspace" aria-label="Workspace"><option value="demo"${mode === 'demo' ? ' selected' : ''}>Demo · this browser</option><option value="shared"${mode === 'shared' ? ' selected' : ''}>Private · shared database</option></select></div></div>`;
}
...
function renderOverview(w) {
  ...
  return `<div class="rei-heading"><div><h2>Know what is visible. Verify what changed.</h2><p class="muted">Current cabinet status, including stores that have not been visited.</p></div>${can(client.executionActor(), 'audit.capture') ? button('tw-new-audit', '+ Start audit', '', 'btn--primary') : ''}</div>
    ...
    ${!w.fixtures.length ? `<div class="card"><h3>Set up the Taiwan workspace</h3><p>Import an approved catalogue and fixture list, then publish and assign a planogram.</p>${button('tw-open-library', 'Open planogram setup')}</div>` : ''}
...
function grid(plan, slots, expected = false) {
  return `<div class="rei-grid" style="--columns:${plan.columns}">${plan.slots.map(s => {
    ...
    const symbol = expected ? '' : status === 'pass' ? '✓' : status === 'fail' ? '!' : '?';
...
export function onAction(action, el, ctx) {
  ...
  return perform(ctx, async () => {
    if (action === 'tw-reset') { if (confirm('Reset only this browser’s synthetic Taiwan example?')) { await client.resetDemo(); state.captureId = null; state.planId = null; state.filter = ''; state.openIssue = null; state.message = 'Prepared example restored. Shared data is unchanged.'; } }
      ...
      if (!w.catalogue.length) throw new Error('Import the Taiwan catalogue and fixtures first.');
