...
export function onAction(action, el, ctx) {
  ...
  return perform(ctx, async () => {
    if (action === 'tw-reset') { if (confirm('Reset only this browser’s synthetic planogram example?')) { await client.resetDemo(); state.captureId = null; state.planId = null; state.filter = ''; state.openIssue = null; state.message = 'Prepared example restored. Shared data is unchanged.'; } }
